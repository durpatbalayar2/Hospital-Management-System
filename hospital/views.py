...
def Login(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:  # Check if user exists
            ...
            error = "yes"  # User is None (authentication failed)
    d = {'error': error}
...
def View_Appointment(request):
    ...
    apt = Appointment.objects.all().select_related('doctor', 'patient')
    
    # Debug information
    print("Number of appointments:", apt.count())
    for appointment in apt:
        print(f"Appointment ID: {appointment.id}")
        print(f"Doctor: {appointment.doctor.name if appointment.doctor else 'None'}")
        print(f"Patient: {appointment.patient.name if appointment.patient else 'None'}")
        print(f"Date: {appointment.date1}")
        print(f"Time: {appointment.time1}")
        print("---")
...
def Add_Appointment(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not doctor:
                error = "yes"
                print(f"Doctor not found: {d}")
            elif not patient:
                error = "yes"
                print(f"Patient not found: {p}")
            else:
                # Create appointment
